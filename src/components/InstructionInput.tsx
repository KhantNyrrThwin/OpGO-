...
export default function InstructionInput() {
  ...
  const validateInstructions = (text: string): ValidationError[] => {
    ...
    instructionLines.forEach((line, index) => {
      ...
      const validInstructions = ['mov', 'mvi', 'jmp', 'jnz', 'jz', 'jnc', 'subi', 'muli', 'mul', 'div', 'jp', 'jm', 'jc', 'inr', 'dcr', 'and', 'andi', 'or', 'divi', 'cmp', 'cpi','ori','xor','xori','not', 'addc', 'addi', 'sub', 'subb','add','lda','sta'];
      
      if (instruction.length > 0) {
        const instructionType = instruction.split(' ')[0];
        if (!validInstructions.includes(instructionType)) {          validationErrors.push({
            line: index,
            message: `Line ${index + 1}: Invalid instruction "${instructionType}". Valid instructions: MOV, MVI, DIVI, AND, ANDI, OR, ADDC, ADDI, SUB, SUBB,ADD,LDA,STA`,
