...
export function executeSUBI(instruction: string, registers: Registers, flags: Flags): { registers: Registers; flags: Flags } {
    ...
    const match = /^subi\s+([0-9a-f]{2})h$/i.exec(trimmed);
    if (!match) {
    ...
    // window.alert(`SUBI: Accumulator before = ${accHex} (${accValue}), Immediate = ${byte} (${immediateValue})`);

    // Perform subtraction (A - immediate)
    let result = accValue - immediateValue;
    if (result < 0) result += 0x100; // Wrap around for 8-bit

    // window.alert(`SUBI: Accumulator after = ${result.toString(16).toUpperCase().padStart(2, '0')} (${result})`);
    ...
    // You may want to set CY flag if accValue < immediateValue, etc.
