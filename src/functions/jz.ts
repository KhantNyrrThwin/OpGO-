/**
 * JZ Instruction Implementation for 8085 Microprocessor
 * 
 * JZ label - Jump to specified label only if Zero flag is 1
 * Example: JZ LOOP
 * 
...
export function executeJZ(
  ...
  // Only proceed if Zero flag is 1
  if (flags.zero === 0) {
    return { registers, flags }; // No jump
  }

  const labelMatch = /^jz\s+([a-z_][a-z0-9_]*)$/i.exec(trimmed);
  if (labelMatch) {
  ...
  console.warn('JZ failed to match or resolve:', instruction);
